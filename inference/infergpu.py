"""
infer.py — Sensory Spotlight (GPU/MPS inference)
================================================
Uses Apple GPU (MPS) on Mac if available, CUDA on NVIDIA systems if available,
...
def extract_audio_features(audio_window: np.ndarray, sr: int = SAMPLE_RATE) -> np.ndarray:
    ...
    return np.concatenate(
        [mfcc_mean, mfcc_std, [rms_mean, rms_max, rms_var], [flux], [zcr]]
    ).astype(np.float32)
...
class SensorySpotlight:
    def __init__(self, visual_ckpt: str, audio_ckpt: str,
                 threshold: float = 0.1, device: torch.device = torch.device("cpu")):
        self.device = device

        print(f"Using device: {self.device}")
        print("Loading backbone ...", end=" ", flush=True)
        self.backbone = load_backbone(self.device)
        print("done")

        self.vis_mlp = VisualMLP().to(self.device)
        self.vis_mlp.load_state_dict(torch.load(visual_ckpt, map_location=self.device))
        self.vis_mlp.eval()

        self.aud_mlp = AudioMLP().to(self.device)
        self.aud_mlp.load_state_dict(torch.load(audio_ckpt, map_location=self.device))
        self.aud_mlp.eval()
    ...
    def _audio_features(self, window: np.ndarray) -> torch.Tensor:
        feats = extract_audio_features(window)
    ...
    def step(self, frame: Image.Image, audio_window: np.ndarray) -> dict:
        # Optional sync for cleaner timing on GPU/MPS
        ...
        return {
            ...
            "aud_error_norm": round(aud_err_norm, 4),
...
def run_on_files(engine, frame_dir: str, wav_path: str,
    ...
    fieldnames = [
        "t", "frame", "channel", "vis_avail", "aud_avail",
